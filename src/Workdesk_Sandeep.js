...
import './Workdesk.css';
import CardUnconcluded from "./CardsWorkdesk/CardUnconcluded";
import CardRunningCall from "./CardsWorkdesk/CardRunningCall";
...
class Workdesk extends Component {
  ...
  componentDidMountWithToken = async () => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (jsonMsg.event_type === "NEW_CALL") {
        ...
        updatedData.unshift(JSON.parse(evt.data));
  ...
  render() {
    ...
    if (this.state.Loading) {
      ...
      showcallRecords = (
        <ul
          // class=" nav-stacked flex-column"
          style={{ flex: 1, scrollSnapType: "y mandatory",textDecoration:"none",listStyle:"none",padding:"0px" }}
          // role="tablist"
        >
          {this.state.new_calls.map((k,index) => (
          <CardRunningCall key={index} element = {k} clicked ={(convId,centerName,callerId)=>this.getConvdata(convId,centerName,callerId)} />

          ))}

          {this.state.unconcluded_calls.map((k,index) => (
            <CardUnconcluded key={index} element = {k} clicked ={(convId,centerName,callerId)=>this.getConvdata(convId,centerName,callerId)} />
          ))}
        </ul>
      );
    }
